...
ostream& operator>>(ostream &outstream, Money &right);
...
ostream& operator<<(ostream &outstream, const Money &right) {
    ...
    outstream << "$" << right.toCents()/100 << endl;
...
istream& operator>>(istream &instream, Money &right) {
    ...
    //TODO: make sure that "$" is entered
    instream >> readsign;
    if (readsign == '$') {
        cout << "That's not a dollar sign or '$'... please try again!" << endl;
    } else {
        //TODO: read money value
        instream >> dollarVal;
    }
